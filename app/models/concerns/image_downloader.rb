...
module ImageDownloader
  ...
  private
    ...
    def extract_dominant_color(image_path)
      # Resize and reduce colors using MiniMagick
      ...
      # Get histogram using Open3 for safe command execution (no shell interpolation)
      result, _status = Open3.capture2("magick", image.path, "-format", "%c", "histogram:info:-")
      ...
      if result =~ /#([0-9A-Fa-f]{6})/
        ...
        Rails.logger.warn("Could not parse dominant color from histogram: #{result.truncate(100)}")
